...
- Processes of each console session
```powershell
Get-NtProcess -InfoOnly | Group-Object SessionId
```

- Only one physical console with RDP for remote desktops
- BaseNamedObjects directories are created per console to avoid name conflicts
	-  `\Sessions\<N>\BaseNamedObjects`
	- Same for Window Stations at `Windows`
- 